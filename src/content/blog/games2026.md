---
title: "10 Games that caught my eye in 2026"
theme: "sunset"
description: "Indie | Games | Upcoming"
date: 09/12/2026
cardBackground: "images/blog/games2026/games2026.webp"
backgrounds:
  - image: "images/blog/games2026/parabellum(0).webp"
  - image: "images/blog/games2026/bigcatch(0).webp"
  - image: "images/blog/games2026/swordhero(0).webp"
  - image: "images/blog/games2026/rogueflight(0).webp"
  - image: "images/blog/games2026/grindset(0).webp"
  - image: "images/blog/games2026/carcassclad(0).webp"
  - image: "images/blog/games2026/ddd(0).webp"
  - image: "images/blog/games2026/afu(0).webp"
  - image: "images/blog/games2026/gstring(0).webp"
  - image: "images/blog/games2026/vaproxy(0).webp"
backgroundBlur: 0.33
backgroundOpacity: 0.9
backgroundParallax: 1

cards:

- tag: ""
  title: ".45 PARABELLUM BLOODHOUND"
  description: ""
  actionText: "Open store"
  backgroundImage: ""
  links:
    - image: "steam"
      href: "https://store.steampowered.com/app/3014650/45_PARABELLUM_BLOODHOUND__Cyberpunk_Active_Time_Action/"
      text: "Steam"
- tag: ""
  title: "The Big Catch"
  description: ""
  actionText: "Open store"
  backgroundImage: ""
  links:
    - image: "steam"
      href: "https://store.steampowered.com/app/2146290/The_Big_Catch/"
      text: "Steam"
- tag: ""
  title: "Sword Hero"
  description: ""
  actionText: "Open store"
  backgroundImage: ""
  links:
    - image: "steam"
      href: "https://store.steampowered.com/app/1739080/Sword_Hero/"
      text: "Steam"
    - image: "youtube"
      href: "https://www.youtube.com/@forestwaregames"
      text: "Developer’s YouTube Channel"
- tag: ""
  title: "ROGUE FLIGHT"
  description: ""
  actionText: "Open store"
  backgroundImage: ""
  links:
    - image: "steam"
      href: "https://store.steampowered.com/app/2784620/ROGUE_FLIGHT/"
      text: "Steam"
- tag: ""
  title: "Grindset T.V."
  description: ""
  actionText: "Open store"
  backgroundImage: ""
  links:
    - image: "steam"
      href: "https://store.steampowered.com/app/3159750/Grindset_TV/"
      text: "Steam"
- tag: ""
  title: "Carcass Clad"
  description: ""
  actionText: "Open store"
  backgroundImage: ""
  links:
    - image: "steam"
      href: "https://store.steampowered.com/app/3327430/Carcass_Clad/"
      text: "Steam"
- tag: ""
  title: "Deep Dish Dungeon"
  description: ""
  actionText: "Open store"
  backgroundImage: ""
  links:
    - image: "steam"
      href: "https://store.steampowered.com/app/2871520/Deep_Dish_Dungeon/"
      text: "Steam"
- tag: ""
  title: "Airframe Ultra"
  description: ""
  actionText: "Open store"
  backgroundImage: ""
  links:
    - image: "steam"
      href: "https://store.steampowered.com/app/2522120/Airframe_Ultra/"
      text: "Steam"
- tag: ""
  title: "G String"
  description: ""
  actionText: "Open store"
  backgroundImage: ""
  links:
    - image: "steam"
      href: "https://store.steampowered.com/app/1224600/G_String/"
      text: "Steam"
- tag: ""
  title: "V.A Proxy"
  description: ""
  actionText: "Open store"
  backgroundImage: ""
  links:
    - image: "steam"
      href: "https://store.steampowered.com/app/2063390/VA_Proxy/"
      text: "Steam"

scenes:
  - image: "images/blog/games2026/parabellum(1).webp"
  - image: "images/blog/games2026/parabellum(2).webp"
  - image: "images/blog/games2026/parabellum(3).webp"
  - image: "images/blog/games2026/bigcatch(1).webp"
  - image: "images/blog/games2026/bigcatch(2).webp"
  - image: "images/blog/games2026/bigcatch(3).webp"
  - image: "images/blog/games2026/swordhero(1).webp"
  - image: "images/blog/games2026/swordhero(2).webp"
  - image: "images/blog/games2026/swordhero(3).webp"
  - image: "images/blog/games2026/rogueflight(1).webp"
  - image: "images/blog/games2026/rogueflight(2).webp"
  - image: "images/blog/games2026/rogueflight(3).webp"
  - image: "images/blog/games2026/grindset(1).webp"
  - image: "images/blog/games2026/grindset(2).webp"
  - image: "images/blog/games2026/grindset(3).webp"
  - image: "images/blog/games2026/carcassclad(1).webp"
  - image: "images/blog/games2026/carcassclad(2).webp"
  - image: "images/blog/games2026/carcassclad(3).webp"
  - image: "images/blog/games2026/ddd(1).webp"
  - image: "images/blog/games2026/ddd(2).webp"
  - image: "images/blog/games2026/ddd(3).webp"
  - image: "images/blog/games2026/afu(1).webp"
  - image: "images/blog/games2026/afu(2).webp"
  - image: "images/blog/games2026/afu(3).webp"
  - image: "images/blog/games2026/gstring(1).webp"
  - image: "images/blog/games2026/gstring(2).webp"
  - image: "images/blog/games2026/gstring(3).webp"
  - image: "images/blog/games2026/vaproxy(1).webp"
  - image: "images/blog/games2026/vaproxy(2).webp"
  - image: "images/blog/games2026/vaproxy(3).webp"
  - image: "images/blog/games2026/games2026.webp"
---

I constantly find myself having a harder time enjoying most popular video games, and not for lack of trying. *Vampire Survivors, Clair Obscure 33, Balatro, Peak, I tried, but they are just not for me at all*, same with most AAA productions. Thankfully, diversity is a strength in the video game industry, so here are games that have caught my attention and why…


[background[0]]

## .45 PARABELLUM BLOODHOUND - Cyberpunk Active Time Action

Based on the trailer alone, there is a lot that I appreciate from the game:

<ol>

  <li>First, the PSX graphics with a gorgeous color palette and special effects (see the raindrop texture on the camera).</li>

  <li>The combat system reminds me of Vagrant Story.</li>

  <li>The music transmits a darkness and fast-pace action, urgency, 2000's vibes.</li>

</ol>

Most of my attention was drawn by the cinematic flair of the game; In just a few scenes, I can tell the author or whoever is in charge of the cinematics/cutscenes has a good understanding and employment of cinematic language. There is excellent blocking, camera angles, use of lighting, and perspective- far, far better than most AAA games I’ve played lately. Fixed camera angles allow for what in my opinion is a greater sense of scale and gameplay, both for puzzles and combat as it is an extra layer of constraint that the player has to work around.

Hopefully this **active time action JRPG** comes out next year!

[scene[0], "Great use of colors and composition, background - foreground. The character shares reflective color while not taking all the attention from the image."]
[scene[1], "Tactical combat. A sphere is activated around the player to target enemies and make use of actions."]
[scene[2], "Once again great use of colors and blocking."]

## The Big Catch

[background[1]]

This game reminds me of *Rayman 2 and Banjo Kazooie*, which I played on an N64 emulator when I was 10-12 or so. The trailer exudes adventure, style, speed and fun, with an excellent choice of colors and character design, fast-paced movement, a big and vast world to explore, along a fun and almost mischievous song. 

it’s been a while since I played a **3D exploration platformer / collectathon**, and I can’t wait to step into this unique world.

[scene[3], "Lens flare with PSX graphics; it's a feast for the eyes. Great texture work and vertex painting for shading."]
[scene[4], "Beautiful sunsets and a far away structures in the distance to set the mood. Our skinny character really pops."]
[scene[5], "Kind of character design me loves, strong use of colors, unique silhouette, lots of presence and therefore charm."]

## Sword Hero

[background[2]]

I must admit a *strong dislike for the term Dark Fantasy and everything related to it*, as to me its just an excuse for adults to feel better about consuming fiction, as if darkness, violence, gore, hopelessness, desaturated colors, and slow, forgettable music are a justification for them to not feel shame about their interests.

And *somehow, even though Sword Hero screams of Dark Fantasy, I can’t help but be attracted to it*. The game’s trailer and its description make it clear this is a Gothic inspired game. *Forestware’s solo dev project* has been in development for some 7 years, and it truly looks like an author’s unique and untouched work in the medium. Its scale, full of many complex systems it’s a commendable feat for a solo dev; even before early access, the game already includes:

<ol>

  <li>Lock-picking mini-game.</li>

  <li>Crime and punishment system.</li>

  <li>Parkour | Level traversal.</li>

  <li>Action limb-based combat.</li>

  <li>Smart AI | Enviromental interaction</li>

  <li>World Factions.</li>

  <li>Towns and cities filled with NPCs(routines)</li>

  <li>Equipment | Inventort |Trading system</li>

</ol>

This will surely be a monumental love letter to **old-school role-playing games** that I hope gets released eventually. The developer uploads blogs on development, plans, and details. I say let him cook!

[scene[6], "Minimal, stylized HUD."]
[scene[7], "Class system, typical melee, ranged and mage."]
[scene[8], "Colorful scenery for a dark fantasy game is a welcome change, I'm also a fan of the game's lightning and kind of greasy camera lens shader."]

## ROGUE FLIGHT

[background[3]]

Did you miss Starfox 64, do you need a dose of space **flight shooter**? Say no more!

Just look at the trailer or screenshots from the game, it gets straight to the point: fun gameplay with lots of old-school dopamine-hitting effects, gorgeous vistas, levels filled with enemy ships, a diegetic user interface, and a hypnotizing soundtrack. Give it a try!

[scene[9], "Chaos on a flying shooter as it should be."]
[scene[10], "The colors… so good…"]
[scene[11], "This kind of scenery is what was in my mind on that asteroid level in StarFox 64."]

## Grindset T.V.

[background[4]]

While I haven’t tried LSD yet, this game might be a close or similar experience. **Fast-paced parkour**, a feast for the eyes, with low-poly stylized level design and cool post-processing effects all over the screen; It looks as if hell on earth its made of metal, sulfuric acid, green and red LEDs and dithering. I can’t wait to play this too!

[scene[12], "Low-poly allows for better and clearer game design when it comes to directing paths."]
[scene[13], "Creepy imagery, yes please."]
[scene[14], "More creepiness."]

## Carcass Clad

[background[5]]

A **co-op tank game** with diagetic controls, this game looks filled with dread, hopelessness, sadness, grief, sorrow, [insert sad syniminus], fitting to the war subject, which CoD could learn a thing or two from(not what their audience wants, I know I know...). The fact that there is a narrative, with 3 written characters the players control, each with their own stories and personalities, makes me want to try this even more. 

From *the developers of Mouthwashing*, I wouldn’t be surprised if the game comes with slim and effective dialogue, good cinematography, crazy plot twists, evil characters, hell… I mean the game might take place in hell for all I know. 

A unique experience that I need injected directly into my front cortex a.s.a.p.

[scene[15], "The oppressiveness and tightness of the tank's inside is transmitted just by screenshot, must be very stressful while playing... stress, that's what I like."]
[scene[16], "War torn city ruins, grey skies, depression, right up my alley."]
[scene[17], "Just a cool shot from the Steam page!"]

## Deep Dish Dungeon

[background[6]]

As I mentioned previously, *I don’t like most popular games(uhhh I'm so different), and yes, that includes Minecraft*. I know, I know, that could be considered a sin coming from a game developer, but I just can’t click with them, and yet here this **co-op survival dungeon crawler** caught my attention. From the trailer, I get the impression there is a good level of polish I often find lacking in most online co-op games, I've played or seen. The setting looks interesting, food/cooking is a main mechanic, and it scratches a fantasy I always had: that of going through the mines of Moria, surrounded by dangers, darkness, and hunger, with only your fellow companions as support. It’s coming out soon, just as I’m unemployed (yay, I guess?).

[scene[18], "Firepits to cook and have warmth."]
[scene[19], "Spooky skeletons are a must in a dungeon."]
[scene[20], "Light sources have always been a reliable beacon to guide players."]

## Airframe Ultra

[background[7]]

Looks like *Mario Kart on Coke*. I love the shading, texture work, the vistas, the cyberpunk (ish) settings; just by the game’s Steam thumbnail you can tell Akira was an inspiration, mostyle on the world and the bikes of course, it's a worthy homage. 

I must also recommend this game's soundtrack (which is how I found Airframe Ultra to begin with), it's 2000's electronic euro vibe goes like dirt and nail with the futuristic setting, and vibrant colors. 

It’s a **retro racing game**, with no story (sadly), but gameplay looks great, and it’s full of style; it knows what it wants to be and it achieves it.

[scene[21], "Akira’s red motorcycle on top and at the bottom a cool teal bike."]
[scene[22], "Customization is always appreciated in multiplayer games."]
[scene[23], "Towering buildings, industry factories and pollution on a sunset backdrop, fits the game's aesthetic perfectly. Also lets give some appreciation to that charming old-school user interface."]

## G String

[background[8]]

Not referring to the sexy G-string to wear but instead to another **cyberpunk FPS**, I really don’t know how to go about this one; there is so much I like, the Steam capsule alone caught my eye. Turns out this game took *10 years to develop* ("10 yearsssss!" Matt Smith in House of the Dragon). As a *solo project*, this is a formidable feat; it takes a great level of dedication and, most of all, discipline to achieve. Assets were mostly done by the developer, just that fact makes me want to try it.

By the time this blog is being written, the game has come out, to some recent mixed reviews… but I’ll play it anyways, it’s a feat that few game devs manage to do, and just because of that I’ll give it the benefit of the doubt. This is someone’s vision; in the screenshots and trailer I see a lot of style, a world I want to explore, and gameplay to get proficient at.

[scene[24], "Overcast sky, a blooming sun, evil robots, red lights of evilness, and a desolate city, this screenshot channels the tone of the story to a tee."]
[scene[25], "Cool narrative sequences, looks like a dream sequence which is a storytelling device I love and wish more games used."]
[scene[26], "Once again a massive sci-fi city, pollution, and neon nights. It's so pretty!"]

## V.A Proxy

[background[9]]

A fast paced game **third person action** game, with some of the most unique ideas I’ve seen in a game, such as:

<ol>

  <li>Mimic colossal.</li>

  <li>Parrying nukes.</li>

  <li>Cities built on giant robots.</li>

</ol>

It’s been quite some time in development, but good things take time.

[scene[27], "Flashy responsive combat."]
[scene[28], "Melee and ranged combat."]
[scene[29], "A massive world to explore."]

## And so…

All of the games that caught my interest are either indie or very, very close to AA, in big part thanks to engines and the democratization of game development. That’s the beauty of games: there are all kinds of flavors for different palates. Thankfully, I found a whole course meal; the bad part is the chefs are still halfway through the process. The good part is the time and love I see when I get a peek of the ingredients through the kitchen window is strong, and the smell is so fierce it’s ingrained in my brain. So I say let them cook!